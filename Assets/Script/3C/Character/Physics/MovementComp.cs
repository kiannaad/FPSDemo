...
namespace CGame
{
    public class MovementComp : IComponent, ICharacterPhysicsController
    {
        ...
        public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
        {
            ...
            {
                Vector3 controlForward = Vector3.ProjectOnPlane(
                    owner.ControlRotation * Vector3.forward,
                    up).normalized;
                Vector3 controlRight = Vector3.ProjectOnPlane(
                    owner.ControlRotation * Vector3.right,
                    up).normalized;
                Vector3 worldMovementInput = controlRight * movementInput.x
                    + controlForward * movementInput.z;
                ...
                float accelerationMultiplier = command.SprintRequested
                    ...
                    : 1f;
                horizontalVelocity += Vector3.ClampMagnitude(worldMovementInput, 1f) *
