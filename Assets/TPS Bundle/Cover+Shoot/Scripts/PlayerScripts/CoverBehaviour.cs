...
public class CoverBehaviour : GenericBehaviour
{
	...
	void JumpOverMatchTarget()
	{
		...
		if (behaviourManager.GetAnim.GetCurrentAnimatorStateInfo(0).IsName("Turn180") && behaviourManager.GetAnim.updateMode != AnimatorUpdateMode.AnimatePhysics)
		{
			// Set update mode to animate physics for full rigidbody rotation.
			behaviourManager.GetAnim.updateMode = AnimatorUpdateMode.AnimatePhysics;
		}
		else if (behaviourManager.GetAnim.GetCurrentAnimatorStateInfo (0).IsName ("JumpOverCover"))
		{
			// Set update mode back to normal.
			if (behaviourManager.GetAnim.updateMode == AnimatorUpdateMode.AnimatePhysics)
